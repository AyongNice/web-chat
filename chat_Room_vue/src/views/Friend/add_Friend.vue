<template>
  <div>
    ...
    <div style="width: 100%; height: 100%;">
      <span
        style="display: flex;justify-content: center; font-size: 12px; font-weight: bold; margin-top: 30px;"
      >
        我的Room号：{{ userInfos.email }}
      </span>
    </div>
  </div>
</template>
    ...
    onSearch(values) {
      if (typeof values == "object") {
        ...
        if (this.searchText) {
          ...
          this.$Request_post(
            this.$AXIOS_URL + "/api/friends/searchEmail",
            data
          ).then(res => {
            console.log(res);
            if (res.data) {
        ...
        if (this.$ls.get("userInfos").email != values) {
          ...
          this.$Request_post(
            this.$AXIOS_URL + "/v1/api/friends/searchEmail",
            data
          ).then(res => {
            console.log(res);
